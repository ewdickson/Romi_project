...
class task_fsm:
    ...
    def __init__(self,
        ...
        self.start_sL = 0.0 # Initial wheel encoder readings (for distance tracking)
        self.start_sR = 0.0
        self.start_time = 0.0

    # angle_deg: signed or unsigned angle magnitude (we use abs)
    def _turn_wheel_arc_inplace(self, angle_deg):     # arc length each wheel must travel for a given in-place rotation (about midpoint)
        angle_rad = math.radians(abs(angle_deg))      
        radius = WHEEL_BASE / 2.0
        return radius * angle_rad

    def _capture_refs(self):    # Capture current encoder readings and time to use as reference for distance-based transitions in the current state
        try:
    ...
    def _dist_since_left(self): # Distance traveled by left wheel since _capture_refs() was called (positive forward, negative reverse)
        return float(self.sL_meas.get()) - self.start_sL

    def _dist_since_right(self):
        return float(self.sR_meas.get()) - self.start_sR

    def run(self): # The main loop of the FSM task, which should be called repeatedly by the scheduler. It checks the current state, performs actions, and transitions between states based on sensor inputs and timing.
        while True:
            # publish state
            ...
            # ---- S0 INIT ----
            if self.state == S0_INIT:   # In the initial state, the robot is stopped and waiting for the go signal. Once go is received, it transitions to the first line-following state.
                self.mode.put(MODE_STOP)
                self.leftMotorGo.put(False)
                self.rightMotorGo.put(False)
                self.leftSetpoint.put(0.0)
            ...
            # ---- S1 LF1 ----
            elif self.state == S1_LF1:  # In the first line-following state, the robot uses the line correction signal to adjust its wheel speeds while moving forward. It transitions to the next state after traveling a certain distance along the line.
                if self.state_entry:
                    self._capture_refs() 
                    self.state_entry = False
                self.mode.put(MODE_LINE_FOLLOW) # Set mode to line following, which will cause the line correction signal to be updated by the line sensor task.
                self.leftMotorGo.put(True) 
                self.rightMotorGo.put(True)
                corr = float(self.lineCorr.get())   # Get the current line correction signal, which indicates how much the robot should steer to stay on the line. A positive value means the line is to the right, so the robot should steer right (increase left wheel speed, decrease right wheel speed).
                base = V_LF # Base speed for line following.
                self.leftSetpoint.put(base + corr)  # Adjust left wheel speed based on correction.
                ...
                dist_ok = (avg_dist - start_avg) >= D_LF1_STRAIGHT  # Check if the robot has traveled the required distance along the line to transition to the next state.
                if dist_ok: # If the distance condition is met, transition to the next state (S2_LF2) and set the state_entry flag to True to perform any initialization actions for the new state.
                    self.state = S2_LF2
                    self.state_entry = True
            
            # ---- S2 LF2 ----

            elif self.state == S2_LF2: # In the second line-following state, the robot continues to follow the line but at a slower speed (V_LFCURVE) to prepare for an upcoming curve. It transitions to the next state after traveling a shorter distance along the line.
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
            ...
            elif self.state == S3_LILCURVE: # In the "little curve" state, the robot performs a small in-place turn to adjust its heading. This is done by setting one wheel to move forward and the other to move in reverse at the same speed (V_TURN). The transition to the next state occurs after either wheel has traveled a certain distance, which corresponds to the arc length needed for the desired turn angle.
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
            ...
            elif self.state == S5_RIGHT90:  # In the right 90-degree turn state, the robot performs an in-place turn to the right by setting the left wheel to move forward and the right wheel to move in reverse at the same speed (V_TURN). The transition to the next state occurs after either wheel has traveled a distance corresponding to a 90-degree turn arc length.
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                self.mode.put(MODE_OPEN_LOOP)
            ...
            # ---- S6 BUMP ----
            elif self.state == S6_BUMP: # In the bump state, the robot moves forward at a moderate speed (V_STRAIGHT) while monitoring the bump sensor. If the bump sensor is triggered (bumpFlag becomes True), it immediately stops and transitions to the recovery state (S7_RECOVER) to back up and try to get around the obstacle.
                self.mode.put(MODE_OPEN_LOOP)
                self.leftMotorGo.put(True)
                self.rightMotorGo.put(True)
            ...
            # ---- S7 RECOVER ----
            elif self.state == S7_RECOVER:  # In the recovery state, the robot attempts to back up and get around the obstacle that caused the bump. It does this by reversing both wheels at a moderate speed (V_STRAIGHT) for a short distance (D_RECOVER). 
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                self.mode.put(MODE_OPEN_LOOP)
            ...
            # ---- S10 LF3 ----
            elif self.state == S10_LF3: # In the third line-following state, the robot follows the line at a slower speed (V_LF3) for a short distance. This is intended to focus the robot on the line to prepare for the upcoming turn. 
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                self.mode.put(MODE_LINE_FOLLOW)
            ...
            # ---- S12 LFPP ----
            elif self.state == S12_LFPP:    # In the final line-following state, the robot follows the line at a slower speed (V_LFCURVE) to navigate the slolum, the portion of the track with tight windy turns. The transition to the next state (S13_TURN180) occurs after traveling the specified distance along the line.
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                self.mode.put(MODE_LINE_FOLLOW)
            ...
            elif self.state == S13_TURN180: # In the "180-degree" turn state, the robot performs an in-place turn to reverse its heading. The final section of the course has a curving 90-degree turn into finish location. This state's turn is adjusted to less than 180-degrees so that the robot can shortcut to the finish location by driving a straight secant line of the final curve.
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                    self._turn_arc_target = self._turn_wheel_arc_inplace(170.0) # Calculate the arc length that each wheel must travel for a 170-degree in-place turn, which will be used as the distance threshold for transitioning to the next state. The angle is set to 170 degrees instead of 180 to allow the robot to cut the corner and drive straight to the finish line, which is more faster than following the curve using line following. 
                self.mode.put(MODE_OPEN_LOOP)
                self.leftMotorGo.put(True)
                self.rightMotorGo.put(True)
                self.leftSetpoint.put(-V_TURN)
            ...
            # ---- S14 STRAIGHT FIN ----
            elif self.state == S14_STRAIGHTFIN: # In the final straight segment state, the robot drives straight towards the finish line at a moderate speed (V_STRAIGHT). The transition to the end state (S15_END) occurs after traveling the specified distance. 
                if self.state_entry:
                    self._capture_refs()
                    self.state_entry = False
                self.mode.put(MODE_OPEN_LOOP)
